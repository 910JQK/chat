...
class 执行命令:
    ...
    async def 请求重置密码(用户, 名字, 邮箱):
        尝试 = await 操作数据库(lambda: 数据库操作.请求重置(名字, 邮箱))
        if 尝试['结果'] == '失败':
            await 用户.发送消息( (getattr(反馈.重置, 尝试['失败原因']))() )
        else:
            await 用户.发送消息(反馈.重置.请求成功())
            await 用户.发送消息(消息确认.重置请求成功(名字))

    async def 重置密码(用户, 名字, 令牌值, 新密码):
        if await 操作数据库(lambda: 数据库操作.重置密码(名字, 令牌值, 新密码)):
            await 用户.发送消息(反馈.重置.重置成功())
        else:
            await 用户.发送消息(反馈.重置.重置失败())
